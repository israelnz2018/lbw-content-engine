...
- **Publish Reel** -- Upload an MP4 and its approved JPG cover, then publish it as a Reel and share it to the Feed
...
Todo Reel deve ter uma capa JPG aprovada. O publicador não aceita publicação sem `--cover`, evitando que o Instagram escolha automaticamente um quadro vazio ou uma transição.

```powershell
node --env-file=.env skills/instagram-publisher/scripts/publish-reel.js `
  --video "C:/caminho/reel.mp4" `
  --cover "C:/caminho/instagram-cover.jpg" `
  --caption-file "C:/caminho/legenda.txt"
```
